...
if (bytes >= 256)
{
  ...
  while (bytes >= 256) {
    ...
    uint64_t in89 = ((uint64_t)in8) | (((uint64_t)in9) << 32);
#if 0
...
#define ONEQUAD_SHUFFLE(a,b,c,d)                \
    ...
    *(uint32_t *) (out + 12) = in##d;                \
    ...
    in##c ^= *(uint32_t *) (m + 72);                 \
    in##d ^= *(uint32_t *) (m + 76);                 \
    *(uint32_t *) (out + 64) = in##a;                \
    *(uint32_t *) (out + 68) = in##b;                \
    *(uint32_t *) (out + 72) = in##c;                \
    *(uint32_t *) (out + 76) = in##d;                \
    in##a = _mm_cvtsi128_si32(z##a);            \
    in##b = _mm_cvtsi128_si32(z##b);            \
    in##c = _mm_cvtsi128_si32(z##c);            \
    in##d = _mm_cvtsi128_si32(z##d);            \
    ...
    in##c ^= *(uint32_t *) (m + 136);                \
    in##d ^= *(uint32_t *) (m + 140);                \
    *(uint32_t *) (out + 128) = in##a;               \
    *(uint32_t *) (out + 132) = in##b;               \
    *(uint32_t *) (out + 136) = in##c;               \
    *(uint32_t *) (out + 140) = in##d;               \
    in##a = _mm_cvtsi128_si32(z##a);            \
    in##b = _mm_cvtsi128_si32(z##b);            \
    in##c = _mm_cvtsi128_si32(z##c);            \
    in##d = _mm_cvtsi128_si32(z##d);            \
    ...
    in##c ^= *(uint32_t *) (m + 200);                \
    in##d ^= *(uint32_t *) (m + 204);                \
    *(uint32_t *) (out + 192) = in##a;               \
    *(uint32_t *) (out + 196) = in##b;               \
    *(uint32_t *) (out + 200) = in##c;               \
    *(uint32_t *) (out + 204) = in##d
